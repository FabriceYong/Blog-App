  ...
  return (
    <div className="dark:text-gray-200">
      ...
      <main className='p-3 flex flex-col max-w-6xl mx-auto min-h-screen'>
        ...
        <Link to={`/search?category=${post?.category}`} className='self-center my-5'>
            <Button color='failure' outline pill size={'xs'}>{post?.category}</Button>
        </Link>
        <img src={post?.image} alt={post?.title} className='mt-5 p-3 max-h-[600px] w-full object-cover' />
